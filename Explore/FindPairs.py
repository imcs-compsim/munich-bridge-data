"""
See presentation on ~/Spaces/DTEC-Netzwerk Digitale Brücke/2022-11-17 Präsentationen Digitale Brücke/2022_11_17_Digitale_Brücke_2_Tarik_Sahin.pdf
for results.
"""
...
file_names = ['../Settlements/UniBw_'+date+'_ref_support_load_01',
...
for i,df_name in enumerate(data.keys()):
    ...
    df_sliced_cross_1[df_sliced_name] = df[start1_dict[df_name]:start1_dict[df_name]+incr].copy()
    df_sliced_cross_2[df_sliced_name] = df[start2_dict[df_name]:start2_dict[df_name]+incr].copy()
...
if plot_red_lines:
    ...
    for i,df_name in enumerate(data.keys()):
        ...
        ax[i].axvline(df.iloc[start1_dict[df_name],0], color='r', linestyle='--', lw=2)
        ax[i].axvline(df.iloc[start1_dict[df_name]+incr,0], color='r', linestyle='--', lw=2)
        ax[i].axvline(df.iloc[start2_dict[df_name],0], color='r', linestyle='--', lw=2)
        ax[i].axvline(df.iloc[start2_dict[df_name]+incr,0], color='r', linestyle='--', lw=2)
...
def generate_datagrid(grid_points,data_matrix,scaled=False):
    if scaled:
        ...
        grid_points_std = (grid_points - grid_points.min()) / (grid_points.max() - grid_points.min())
        data_matrix_std = (data_matrix - data_matrix.min()) / (data_matrix.max() - data_matrix.min())
...
if plot_sliced_data:
    ...
    for i,df in enumerate(df_sliced_cross_1):
        axs[0,i].plot(df_sliced_cross_1[df].iloc[:,0],df_sliced_cross_1[df].iloc[:,1])
        axs[0,i].xaxis.set_major_formatter(formatter)
        axs[0,i].xaxis.set_major_locator(MaxNLocator(5))
        plt.setp(axs[0,i].xaxis.get_ticklabels(), rotation=25, ha="right", rotation_mode="anchor")
    ...
    for i,df in enumerate(df_sliced_cross_2):
        axs[1,i].plot(df_sliced_cross_2[df].iloc[:,0],df_sliced_cross_2[df].iloc[:,1])
        axs[1,i].xaxis.set_major_formatter(formatter)
        axs[1,i].xaxis.set_major_locator(MaxNLocator(5))
        plt.setp(axs[1,i].xaxis.get_ticklabels(), rotation=25, ha="right", rotation_mode="anchor")
        if calculate_fisher_rao_distance:
    ...
    for (i, j), z in np.ndenumerate(fisher_global):
        ax.text(j, i, '{:0.2f}'.format(z), ha='center', va='center', fontsize=17)
    tickets = ["Ref_1", "Settl_1\n (1 cm)", "Settl_1\n (2 cm)", "Settl_1\n (3 cm)", "Ref_2", "Settl_2\n (1 cm)", "Settl_2\n (2 cm)", "Settl_2\n (3 cm)"]
...
calculate_fisher_rao_distance = True
smoother = ks.LocalLinearRegressionSmoother(smoothing_parameter = 0.05) #0.05 -->scaling 4 -->
...
if plot_sliced_smooth_data:
    ...
    for i,df in enumerate(df_sliced_cross_1):
        ...
        axs[0,i].plot(df_sliced_cross_1[df].iloc[:,0],fd.data_matrix[0].flatten())
        axs[0,i].xaxis.set_major_formatter(formatter)
        axs[0,i].xaxis.set_major_locator(MaxNLocator(5))
        plt.setp(axs[0,i].xaxis.get_ticklabels(), rotation=25, ha="right", rotation_mode="anchor")
    ...
    for i,df in enumerate(df_sliced_cross_2):
        ...
        axs[1,i].plot(df_sliced_cross_1[df].iloc[:,0],fd.data_matrix[0].flatten())
        axs[1,i].xaxis.set_major_formatter(formatter)
        axs[1,i].xaxis.set_major_locator(MaxNLocator(5))
        plt.setp(axs[1,i].xaxis.get_ticklabels(), rotation=25, ha="right", rotation_mode="anchor")
